...
import type { HuntSession } from "../schemas/hunt-session.schema";
...
type DbGetHuntSessionPayload = {
  supabase: TypedSupabaseClient;
  id: HuntSession["id"];
  character_id: CharacterID;
};

export function dbGetHuntSession({ supabase, id, character_id }: DbGetHuntSessionPayload) {
  return supabase
    .from("hunt_sessions")
    .select(SELECT)
    .eq("id", id)
