...
type DbDeleteHuntSessionPayload = {
  supabase: TypedSupabaseClient;
  id: number;
};

export function dbDeleteHuntSession({ supabase, id }: DbDeleteHuntSessionPayload) {
  return supabase.from("hunt_sessions").delete().eq("id", id);
}

type DbInsertHuntSessionPayload = {
  supabase: TypedSupabaseClient;
  payload: CreateHuntSessionPayload;
};

export function dbInsertHuntSessionRPC({ supabase, payload }: DbInsertHuntSessionPayload) {
...
type DbUpdateHuntSessionPayload = {
  supabase: TypedSupabaseClient;
  payload: UpdateHuntSessionPayload;
};

export async function dbUpdateHuntSessionRPC({ supabase, payload }: DbUpdateHuntSessionPayload) {
